...
/**
 * The tool-routing + voice-rules persona fragment, as a function of whether
 * Google Search grounding is declared. ONE builder for both the hosted
 * profile's compile model and the runtime wiring, so persona and tool wiring
 * cannot drift — and the search flag flips the routing line and the tool
 * together (design G9: the model never routes to a tool it can't call).
 */
export function buildSutandoPersonaFragment(googleSearch: boolean): string {
	return [
		'TOOL ROUTING:',
...
export function createSutandoAgentConfig(
	...
	const personaFragment = buildSutandoPersonaFragment(options.googleSearch !== false);
